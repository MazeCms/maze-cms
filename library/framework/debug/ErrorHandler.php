 ...
 *
 */
class ErrorHandler extends \maze\base\ErrorHandler
{
    /**
    ...
    /**
     * @var string - шаблон темы страницы ошибки
     */
    public $errorLayout = '//error';
    /**
     * @var string - шаблон страницы ошибки по умолчанию
     */
    public $errorView = '@root/admin/templates/system/error/tmp.error.php';
    /**
     * @var string the path of the view file for rendering exceptions.
     */
    public $exceptionView = '@root/admin/templates/system/error/tmp.exception.php';
    /**
     * @var string the path of the view file for rendering exceptions and errors call stack element.
     */
    public $callStackItemView = '@root/admin/templates/system/error/tmp.callStackItem.php';
    /**
     * @var string the path of the view file for rendering previous exceptions.
     */
    public $previousExceptionView = '@root/admin/templates/system/error/tmp.previousException.php';
    ...
    protected function renderException($exception)
    {
        ...
        if (RC::app()->getResponse()) {
            $response = RC::app()->getResponse();
        } else {
            $response = new Response();
        }
        
        $useErrorView = $response->format === Response::FORMAT_HTML && (!DEBUG || $exception instanceof UserException);
        ...
        if ($useErrorView && $this->errorLayout !== null && $view && $view->hasView($this->errorLayout)) {
            
            $view->layout = $this->errorLayout;
    ...
    /**
     * Конвертируем стек ошибок в массив для ajax запросов
     * @param \Exception $exception the exception being converted
     * @return array the array representation of the exception.
     */
    protected function convertExceptionToArray($exception)
    {
        if (!DEBUG && !$exception instanceof UserException && !$exception instanceof HttpException) {
        ...
        if (DEBUG) {
            ...
            if (!$exception instanceof UserException) {
                ...
                if ($exception instanceof \maze\exception\DBException) {
    ...
    {
        return htmlspecialchars($text, ENT_QUOTES, RC::app()->config->charset);
    }
    ...
    public function addTypeLinks($code)
    {
        ...
        if (strpos($code, 'maze\\') !== 0) {
            return $text;
        }

        $page = $this->htmlEncode(strtolower(str_replace('\\', '-', $class)));
        $url = "http://www.maze-studio.ru/doc/$page.html";
        if (isset($method)) {
            $url .= "#$method()-detail";
        }
    ...
    public function renderFile($_file_, $_params_)
    {
        ...
        if ($this->exception instanceof ErrorException || !RC::app()->getView()) {
            ob_start();
            ob_implicit_flush(false);
            extract($_params_, EXTR_OVERWRITE);
            require(RC::getAlias($_file_));

            return ob_get_clean();
        } else {
            return RC::app()->getView()->renderFile($_file_, $_params_, $this);
        }
    }
    ...
    {
        return $file === null || strpos(realpath($file), PATH_LIBRARIES.DS.'framework') === 0;
    }
    
    public function isExpFile($file){
        return $file === null || strpos(realpath($file), PATH_SITE.DS.'expansion') === 0;
    }
    ...
    public function getExceptionName($exception)
    {
        ...
        if ($exception instanceof \maze\exception\Exception) {
