...
class BaseHtml
{
    ...
    public static function beginForm($action = '', $method = 'post', $options = [])
    {
        ...
        $action = $action ? new \URI($action) : \URI::instance();

        $hiddenInputs = [];

        if (!strcasecmp($method, 'get') && $action->hasVars()) {      
            foreach ($action->getQuery(true) as $name=>$val) {
                $hiddenInputs[] = static::hiddenInput(urldecode($name),urldecode($val));
            }
        ...
        $options['action'] = $action->toString(['path', 'query', 'fragment']);
    ...
    {
        $options['src'] = (new \URI($src))->toString();
    ...
    public static function errorSummary($models, $options = [])
    {
        ...
        $header = isset($options['header']) ? $options['header'] : '<p>Error:'. '</p>';
