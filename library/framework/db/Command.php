...
namespace maze\db;
use maze\exception\DBException;
 ...
 * @since 2.0
 */
class Command extends \maze\base\Object
{
    /**
     ...
     */
    public function cache($duration = null, $type = null)
    {
 
        $this->queryCacheDuration = $duration === null ? $this->db->queryCacheDuration : $duration;
        $this->queryCacheType = $type;
    ...
    public function execute()
    {
        ...
        try {
            $logDb = \RC::getLog()->add('db', ['query'=>$token, 'category'=>__METHOD__]);
            $logDb->beginQuery();
            $this->prepare();
    ...
    private function queryInternal($method, $fetchMode = null)
    {
        ...
        if ($method !== '') {
            ...
            $info = $this->db->getQueryCacheInfo($this->queryCacheDuration, $this->queryCacheType);
           
            if (is_array($info)) {   
                  
                /* @var $cache объект кеша тип queryCacheType*/
                $cache = $info[0];
                $cacheKey = [
                    __CLASS__,
                    $method,
                ...
                if (($result = $cache->get($cacheKey)) !== false) {
                    ...
                    \RC::getLog()->add('cache', ['text'=>$rawSql, 'type'=>'read', 'category'=>'db', 'group'=>$info[2]]);
                    return $result;
                }
            }
        }
        try {
            $logDb = \RC::getLog()->add('db', ['query'=>$rawSql, 'category'=>__METHOD__]);
            $logDb->beginQuery();
            $this->prepare();
            $this->pdoStatement->execute();
        ...
        if (isset($cache, $cacheKey, $info)) { 
            ...
            $cache->setTimeLive($info[1]);
            $cache->set($cacheKey, $result);
            \RC::getLog()->add('cache', ['text'=>$rawSql, 'type'=>'write', 'category'=>'db', 'group'=>$info[2]]);
        }
