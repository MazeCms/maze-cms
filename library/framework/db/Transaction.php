 ...
 * @since 2.0
 */
class Transaction extends \maze\base\Object
{
    /**
    ...
    public function begin()
    {
        ...
        if ($this->_level == 0) {
            //Yii::trace('Begin transaction', __METHOD__); логи
        ...
        if ($schema->supportsSavepoint()) {
            //Yii::trace('Set savepoint ' . $this->_level, __METHOD__); логи
            $schema->createSavepoint('LEVEL' . $this->_level);
        } else {
            //Yii::info('Transaction not started: nested transaction not supported', __METHOD__); логи
        }
        $this->_level++;
    }
    ...
    public function commit()
    {
        ...
        $this->_level--;
        if ($this->_level == 0) {
            //Yii::trace('Commit transaction', __METHOD__); логи
            $this->db->pdo->commit();
        ...
        $schema = $this->db->getSchema();
        if ($schema->supportsSavepoint()) {
           // логи Yii::trace('Release savepoint ' . $this->_level, __METHOD__);
            $schema->releaseSavepoint('LEVEL' . $this->_level);
        } else {
         // логи   Yii::info('Transaction not committed: nested transaction not supported', __METHOD__);
        }
    }
    ...
    public function rollBack()
    {
        ...
        $this->_level--;
        if ($this->_level == 0) {
            // логи Yii::trace('Roll back transaction', __METHOD__);
