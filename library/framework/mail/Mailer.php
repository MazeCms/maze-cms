 ...
 *
 * ~~~
 * RC::getMail()->compose('contact/html', ['contactForm' => $form])
...
class Mailer extends BaseMailer
{
     ...
     */
    public $messageClass = 'maze\mail\Message';
    ...
    protected function sendMessage($message)
    {
        ...
        RC::getLog()->add('exp',['component'=>'system',
                        'category'=>__METHOD__,
                        'action'=>'sendMessage',
                        'status'=>'success',
                        'message'=>'Sending email "' . $message->getSubject() . '" to "' . $address]);
    ...
    protected function createSwiftObject(array $config)
    {
        ...
        if (isset($config['constructArgs'])) {
            ...
            $object = RC::createObject($className, $args);
        } else {
            $object = RC::createObject($className);
        }
        if (!empty($config)) {
