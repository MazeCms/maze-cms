...
if ($widget->field->many_value !== 0 && $widget->field->many_value !== 1) {
    $widget->clientOptions['onAfterAdd'] = new JsExpression('function(){$(this).find("input").each(function(i){ $(this).attr("name", "' . $widget->field->field_name . '["+i+"][text_value]' . '")})}');
    $widget->clientOptions['update'] = new JsExpression('function(){$(this).find("input").each(function(i){ $(this).attr("name", "' . $widget->field->field_name . '["+i+"][text_value]' . '")})}');
            ...
            <?php foreach ($widget->data as $id => $data): ?>
                <tr>
                    ...
                    <td class="text-cente align-middle">
                        ...
                        <?= Html::textInput(Html::getInputName($data, "[$id]text_value"), Html::getAttributeValue($data, "[$id]text_value"), ['class' => 'form-control', 'id' => Html::getInputId($data, "[$id]text_value")]); ?>
...
<?php else: ?>
    ...
    <?= Html::textInput(Html::getInputName($widget->data[0], "text_value"), Html::getAttributeValue($widget->data[0], "text_value"), ['class' => 'form-control', 'id' => Html::getInputId($widget->data[0], "text_value")]); ?>
