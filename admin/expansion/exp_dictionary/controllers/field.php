...
class Dictionary_Controller_Field extends Controller {

    public function accessFilter() {
        return [
            'add edit publish unpublish display field sort' => ["dictionary", "EDIT_FIELD"],
            'delete' => ["dictionary", "DELETE_FIELD"]
        ];
    }
    ...
    public function actionDisplay() {
        ...
        if ($this->request->isAjax() && $this->request->get('clear') == 'ajax') {
            ...
            $model = ContentType::find()->from(['ct'=>ContentType::tableName()])
                    ->joinWith('field')->where(['ct.expansion' => 'dictionary']);
     
            return (new GridFormat([
                'id' => 'dictionary-field-type-grid',
                'model' => $model,
                'colonum' => 'bundle',                
                'colonumData' => [
                    'bundle',
                    'title'=>function($data){
                        return Html::a($data->title, ['/admin/dictionary/field', ['run'=>'field', 'bundle'=>$data->bundle]]);
                    },
                    'description',
    ...
    public function actionField($bundle){
        if ($this->request->isAjax() && $this->request->get('clear') == 'ajax') {
            ...
            $model = FieldExp::find()->from(['fe'=>FieldExp::tableName()])
                    ->joinWith('typeFields')->where(['fe.expansion' => 'dictionary', 'fe.bundle'=>$bundle]);
     
            return (new GridFormat([
                'id' => 'dictionary-field-grid',
                'model' => $model,
                'colonum' => 'fe.sort',
                'colonumData' => [
    ...
    public function actionAdd($name, $bundle, $widget_name = null) {
       ...
       if ($this->request->isPost()) {
           ...
           if ($this->request->isAjax() && $this->request->get('checkform') == 'dictionary-form-field') {
           ...
           if(maze\base\Model::validateMultiple($models)){
               if($field->save()){
                  $this->setMessage(Text::_("EXP_DICTIONARY_FIELD_ACTION_ADD", ['name'=>$field->title]), 'success');
    ...
    public function actionEdit($field_exp_id, $widget_name = null) {
       ...
       if(!($field = FieldHelper::findByID($field_exp_id))){
           throw new maze\exception\NotFoundHttpException(Text::_("EXP_DICTIONARY_FIELD_NOTID", ['id' => $field_exp_id]));
       }
       
       if($field->locked){
           throw new maze\exception\NotFoundHttpException(Text::_("EXP_DICTIONARY_FIELD_LOCK", ['name'=> $field->type, 'id' => $field_exp_id]));
       }
       if($widget_name !== null){
       ...
       if ($this->request->isPost()) {
           ...
           if(maze\base\Model::validateMultiple($models)){
               if($field->save()){
                  $this->setMessage(Text::_("EXP_DICTIONARY_FIELD_UPDATE", ['name'=>$field->title]), 'success');
                    if ($this->request->get('action') == 'saveClose') {
                        return $this->setRedirect([['run'=>'field', 'bundle'=>$field->bundle]]);
                    }
                    return $this->setRedirect([['run' => 'edit', 'bundle' =>$field->bundle, 'field_exp_id'=> $field->field_exp_id]]); 
    ...
    public function actionPublish(array $field_exp_id, $bundle) {
        ...
        $this->setMessage("EXP_DICTIONARY_FIELD_PUBLISH", 'success');
        if(!$this->request->isAjax()){
            $this->setRedirect(['/admin/dictionary/field', ['run'=>'field', 'bundle'=>$bundle]]);
        }
    }

    public function actionUnpublish(array $field_exp_id, $bundle) {
        FieldExp::updateAll(['active'=>0], ['field_exp_id'=>$field_exp_id, 'locked'=>0]);
        $this->setMessage("EXP_DICTIONARY_FIELD_UNPUBLISH", 'success');
        if(!$this->request->isAjax()){            
            $this->setRedirect(['/admin/dictionary/field', ['run'=>'field', 'bundle'=>$bundle]]);
        }
    }
    ...
    public function actionDelete(array $field_exp_id, $bundle) {
        ...
        foreach($field_exp_id as $id){
            if(($field = FieldHelper::findByID($id))){
                if($field->findData()){
                    $this->setMessage(Text::_("EXP_DICTIONARY_FIELD_DELETE_ENTRY", ['name' => $field->title]), "error");
                    $result = true;
                    break;
                }elseif($field->locked){
                    $this->setMessage(Text::_("EXP_DICTIONARY_FIELD_DELETE_LOCKED", ['name' => $field->title]), "error");
                    $result = true;
                    break;
                }else{
                    if(!$field->delete()){
                ...
                $this->setMessage(Text::_("EXP_DICTIONARY_FIELD_NOTID", ['id' => $field_exp_id]), "error");
                $result = true;
                break;
            }
        }
        ...
        if(!$result){
            $this->setMessage(Text::_("EXP_DICTIONARY_FIELD_DELETE_OK"), 'success');
        }
