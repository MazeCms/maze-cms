...
(function ($) {
    ...
    function ToolBarAdmin(elem, options)
    {
        this.$element = $(elem);
        this.options = $.extend({}, $.fn.toolBarAdmin.defaults, options || {});
    ...
    ToolBarAdmin.prototype = {
        constructor: ToolBarAdmin,
        init: function ()
        {
            ...
            var $elem = this.$element,
                    menuH = $elem.outerHeight(true),
                    topStart = $elem.offset().top,
                    leftStart = $elem.offset().left;
            ...
            function handlerScroll(e)
            {
                ...
                if ($elem.is('.fixed-bar-mode'))
                {
                    if (e.type == 'scroll')
                    {
                        if ($(document).scrollTop() > topStart)
                        {
                            $elem.width($elem.width()).css({position: 'fixed', top: 0});
                        }
                        else
                        {
                            $elem.removeAttr('style');
                    ...
                    {
                        $elem.width($(window).width() - leftStart);
                    }

                }

            }
            $(document).on('scroll', handlerScroll);
            $(window).on('resize', handlerScroll);
        ...
        fixedBar: function ()
        {
            ...
            if (this.$element.is('.fixed-bar-mode'))
            {
                ...
                $('#fixed-top-bar').find('.icon-tool-bar').toggleClass('bar-icon-clip fixed-active');
        ...
        createMessages: function ()
        {
            ...
            {
               this.$element.append($mess);
                
                this.$element.bind('unfixedlBar', function (e) {
                    if ($mess.is(':visible'))
                    {
        ...
        setValue: function (name, valAs)
        {
            ...
            if (value)
            {
                ...
                if ($.isPlainObject(value))
                {
                    var res = {};
                    res[name] = valAs
                    res = $.extend(value, res);
                    $.cookie('toolBarAdmin', JSON.stringify(res));
                }
            }
            else
            {
                var res = {};
                res[name] = valAs
                $.cookie('toolBarAdmin', JSON.stringify(res), {expires: selfClass.options.expires});
            }
        },
        getValue: function (name)
        {
            ...
            if (value)
            {
                ...
                if ($.isPlainObject(value))
                {
                    ...
                    return value.hasOwnProperty(name) ? value[name] : false;
    ...
    $.fn.toolBarAdmin = function (options) {
        var arg = arguments;

        var $this = $(this);

        var instance = $this.data('toolBarAdmin');
        ...
        if (instance)
        {
            ...
            {
                instance.setMessage(arg[1], arg[2]);
            }
        ...
        {
            instance = new ToolBarAdmin(this, options);
            $this.data('toolBarAdmin', instance);
        }

        return this;
    }

    $.fn.toolBarAdmin.defaults = {
